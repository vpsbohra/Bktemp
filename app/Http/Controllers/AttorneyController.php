...
// 			Mail::to('sam.beatum@gmail.com')->send(new WelcomeAboard($user,false,$attorney,$subscription_video,$thumbnail_video_image));
	...
	public function official_form($id){
		...
		$financialaffairs_info=$this->financialaffairs_information($client_id);
		// dump($basic_info);
        return view('attorney.official_form',['basic_info'=>$basic_info,'property_info'=>$property_info,'income_info'=>$income_info,'debts'=>$debts,'attorney_company'=>$attorney_company,'expenses_info'=>$expenses_info,'financialaffairs_info'=>$financialaffairs_info, 'client_id'=>$client_id]);
	}
